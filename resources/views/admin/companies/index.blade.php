@extends('layouts.app')

@section('title', 'Kelola Perusahaan')

@section('content')
<!-- [ breadcrumb ] start -->
<div class="page-header">
    <div class="page-block">
        <div class="row align-items-center">
            <div class="col-md-12">
                <div class="page-header-title">
                    <h5 class="m-b-10">Kelola Perusahaan</h5>
                </div>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Admin</a></li>
                    <li class="breadcrumb-item" aria-current="page">Kelola Perusahaan</li>
                </ul>
            </div>
        </div>
    </div>
</div>
<!-- [ breadcrumb ] end -->

<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="ti ti-building"></i> Daftar Perusahaan</h5>
                <a href="{{ route('admin.companies.create') }}" class="btn btn-primary">
                    <i class="ti ti-plus"></i> Tambah Perusahaan
                </a>
            </div>
            <div class="card-body">
                @if($companies->isEmpty())
                    <div class="text-center py-5">
                        <i class="ti ti-building" style="font-size: 4rem; color: #ccc;"></i>
                        <h5 class="mt-3 text-muted">Belum ada perusahaan</h5>
                    </div>
                @else
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    <th>Nama Perusahaan</th>
                                    <th>Alamat</th>
                                    <th>Telepon</th>
                                    <th>Email</th>
                                    <th>Jumlah Magang</th>
                                    <th>Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($companies as $company)
                                <tr>
                                    <td><strong>{{ $company->name }}</strong></td>
                                    <td>{{ Str::limit($company->address, 50) ?? '-' }}</td>
                                    <td>{{ $company->phone ?? '-' }}</td>
                                    <td>{{ $company->email ?? '-' }}</td>
                                    <td>
                                        <span class="badge bg-light-primary border border-primary">
                                            {{ $company->internships_count }} mahasiswa
                                        </span>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.companies.show', $company) }}" class="btn btn-sm btn-outline-info">
                                            <i class="ti ti-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.companies.edit', $company) }}" class="btn btn-sm btn-outline-primary">
                                            <i class="ti ti-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.companies.destroy', $company) }}" method="POST" class="d-inline"
                                              onsubmit="return confirm('Apakah Anda yakin ingin menghapus perusahaan ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-outline-danger">
                                                <i class="ti ti-trash"></i>
                                            </button>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                @endif
            </div>
            @if(!$companies->isEmpty())
            <div class="card-footer">
                {{ $companies->links() }}
            </div>
            @endif
        </div>
    </div>
</div>
@endsection
